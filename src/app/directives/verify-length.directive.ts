...
export class VerifyLengthDirective implements Validator {
  ...
  validate(control: AbstractControl): ValidationErrors | null {
    let { min = 0, max = Infinity } = this.verifyLength as {
      min?: number;
      max?: number;
    };
 ...
 * @returns 验证器函数
 */
export function verifyLengthValidator(min = 0, max = Infinity): ValidatorFn {
...
const getValidateResult = (
  ...
  if (TypeJudgementUtil.isArray(controlValue)) {
    // control.value.length（length属性值）
    if (controlValue.length < min || controlValue.length > max)
      return { verifyLength: true };
  }
