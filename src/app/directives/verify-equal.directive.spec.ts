...
import { verifyEqualValidator, VerifyEqualDirective } from './verify-equal.directive';
...
export class TestComponent {
  ...
  userForm = new FormGroup({
    password: new FormControl('', [verifyEqualValidator('confirmPassword', true)]),
...
describe('VerifyEqualDirective Test', () => {
  ...
  it('FormsModule control status', () => {
    ...
    expect(component.myForm.controls['confirmPassword']?.errors).toEqual({ verifyEqual: true });
  ...
  it('ReactiveFormsModule control status', () => {
    const passwordEle = debugElement.query(By.css('#userPassword')).nativeElement;
    passwordEle.value = 'password';
    passwordEle.dispatchEvent(new Event('input'));
    expect(component.userForm.controls['confirmPassword']?.valid).toEqual(false);
    expect(component.userForm.controls['password']?.valid).toEqual(true);
    const confirmPasswordEle = debugElement.query(By.css('#userConfirmPassword')).nativeElement;
    confirmPasswordEle.value = 'password';
    confirmPasswordEle.dispatchEvent(new Event('input'));
